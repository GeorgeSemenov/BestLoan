...
gulp.task('buildImg',['imgWatcher'], function() {
    gulp.src('app/img/**/*')
    .pipe(responsive({
        '**/*.*': [{
            width: 1600,
...
//старое правило конвертации изображений, не знаю зачем оно тут.
// gulp.task('buildImg',['imgWatcher'], function() {
//     gulp.src('app/img/originalImg/**/*')
//     .pipe(responsive({
//         '**/*.*': [{
//             width: 2240,
//             suffix: '-2240',
...
gulp.task('convertImg', function() {
    return gulp.src(['!app/img/originalImg/','!app/img/originalImg/**/*','app/img/**/*'])
        .pipe(cache(imagemin([
            //png
            pngquant({
                speed: 1,
...
gulp.task('build',['clean','convertImg','lessTask','buildScripts','hamlTask'],function(){
